				   ...
				   struct v4l2_subdev_frame_interval *fi)
{
	...
	mutex_unlock(&imx335->mutex);
        ...
        dev_err(&imx335->client->dev, "%s frame rate = %d / %d\n",
             __func__, mode->max_fps.numerator,
...
static int imx335_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	struct i2c_client *client = imx335->client;
	int ret = 0;
	printk("imx335_s_stream add by lwx\n");
	dev_err(&imx335->client->dev, "s_stream: %d. %dx%d, hdr: %d, bpp: %d\n",
