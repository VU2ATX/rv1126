...
export class ConfigService {
  ...
  getVideoEncoderInterface(streamType: string): Observable<VideoEncoderInterface> {
    return this.http
      .get<VideoEncoderInterface>( this.serverUrl + '/video/' + this.getIdFromStreamType(streamType) + '?timestamp=' + new Date().getTime())
      .pipe(catchError(this.handleError));
  }
  ...
  putScreenshotTimingInfo(data: TimingInterface): Observable<TimingInterface> {
    return this.http
      .put<TimingInterface>(this.serverUrl + environment.planTimingUrl, data)
      .pipe(catchError(this.handleError));
  }

  putScreenshotEventInfo(data: EventShotInterface): Observable<EventShotInterface> {
    return this.http
