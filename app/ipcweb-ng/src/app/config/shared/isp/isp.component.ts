...
export class IspComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  updateDynamicCap(cardTitle: string = null, layoutItem: string = null) {
  ...
  onSubmitPart(cardTitle: string, layoutItem: string, type: string) {
    ...
    switch (type) {
      case 'number':
        ...
        break;
      case 'time':
        if (this.imageForm.value['nightToDay']['sEndTime'].match(':') && this.imageForm.value['nightToDay']['sBeginTime'].match(':')) {
          const start = this.imageForm.value['nightToDay']['sBeginTime'].split(':');
          const end = this.imageForm.value['nightToDay']['sEndTime'].split(':');
