...
int FlowEventProc(std::shared_ptr<easymedia::Flow> flow, bool &loop) {
  ...
  while (loop) {
    ...
    if (event == nullptr && msg == nullptr)
      continue;
    LOG_INFO("FlowEventProc flow %x msg id %d param %d\n", (int)flow.get(),
    ...
    case MSG_FLOW_EVENT_INFO_MOVEDETECTION: {
      ...
      if (mdevent) {
        ...
        break;
#endif
        LOG_INFO("@@@ MD: Get movement info[%d]: ORI:%dx%d, DS:%dx%d\n",
...
#endif
          LOG("recognize user_%d,similarity = %f, pic_path = %s\n",
              face_reg->user_id, face_reg->similarity, face_reg->pic_path);
        } else {
          LOG("recognize Unknow, similarity = %f, pic_path = %s\n",
