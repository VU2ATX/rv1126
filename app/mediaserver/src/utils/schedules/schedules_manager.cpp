...
void SchedulesManager::CheckVideoPlan() {
  ...
  for (int j = 0; j < MAX_DAY_SCHEDULE_NUM; j++) {
    std::string type = "";
    if (!schedule_video_->week_day[day_index_].day_period[j].end_minute)
      continue;
    else
...
void SchedulesManager::CheckPhotoPlan() {
  ...
  for (int j = 0; j < MAX_DAY_SCHEDULE_NUM; j++) {
    std::string type = "";
    if (strlen(schedule_photo_->week_day[day_index_].day_period[j].type) == 0)
      continue;
    else
...
void SchedulesManager::CheckMDPlan() {
  ...
  for (int j = 0; j < MAX_DAY_SCHEDULE_NUM; j++) {
    if (!schedule_md_->week_day[day_index_].day_period[j].end_minute)
      continue;
    int start_minute =
...
void SchedulesManager::EventSnap() {
  ...
  if (event_snap_flag_) {
    LOG_INFO("event snapshoot already in progress\n");
    return;
  }
