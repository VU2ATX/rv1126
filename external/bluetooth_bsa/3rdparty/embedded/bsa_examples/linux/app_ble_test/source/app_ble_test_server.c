...
static UINT8 app_ble_test_server_adv_value[APP_BLE_TS_ADV_VALUE_LEN] =
        APP_BLE_TS_ADV_VALUE;

static UINT8 app_ble_test_server_scan_rsp_value[APP_BLE_TS_SCAN_RSP_VALUE_LEN] =
 ...
 * Local functions
 */
static void app_ble_test_server_profile_cback(tBSA_BLE_EVT event,  tBSA_BLE_MSG *p_data);
...
int app_ble_test_server_start(void)
{
    ...
    /* clear control block */
    memset(&app_ble_ts_cb, 0, sizeof(app_ble_ts_cb));

    /* register BLE server app */
    app_ble_server_register(APP_BLE_TS_APP_UUID, app_ble_test_server_profile_cback);
    GKI_delay(1000);

    /* create a BLE service */
    app_ble_test_server_create_service();
    GKI_delay(1000);

    /* add a characteristic */
    app_ble_test_server_add_char(APP_BLE_TS_CHAR_NOTI_UUID);
    app_ble_ts_cb.chars[app_ble_ts_cb.counter].char_uuid = APP_BLE_TS_CHAR_NOTI_UUID;
    GKI_delay(1000);

    /* add a characteristic */
    app_ble_test_server_add_char(APP_BLE_TS_CHAR_INDI_UUID);
    app_ble_ts_cb.chars[app_ble_ts_cb.counter].char_uuid = APP_BLE_TS_CHAR_INDI_UUID;
    GKI_delay(1000);

    /* add a characteristic descriptor */
    app_ble_test_server_add_char_desc(APP_BLE_TS_CHAR_UUID);
    app_ble_ts_cb.chars[app_ble_ts_cb.counter].char_uuid = APP_BLE_TS_CHAR_UUID;
    GKI_delay(1000);

    /* start service */
    app_ble_test_server_start_service();
    GKI_delay(1000);

    /* start advertising */
    memset(&adv_conf, 0, sizeof(tBSA_DM_BLE_ADV_CONFIG));
    adv_conf.len = APP_BLE_TS_ADV_VALUE_LEN;
    adv_conf.flag = BSA_DM_BLE_ADV_FLAG_MASK;
    memcpy(adv_conf.p_val, app_ble_test_server_adv_value, APP_BLE_TS_ADV_VALUE_LEN);
    ...
    scan_rsp.len = APP_BLE_TS_ADV_VALUE_LEN;
    scan_rsp.flag = BSA_DM_BLE_ADV_FLAG_MASK;
    memcpy(scan_rsp.p_val, app_ble_test_server_scan_rsp_value, APP_BLE_TS_SCAN_RSP_VALUE_LEN);
