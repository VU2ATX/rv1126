 ...
 **
 ** Description     This function asks user to freq encoded and sends the VSC.
 **
 ** Parameters      None
 **
 ** Returns         status: 0 if success / -1 otherwise
 **
 *******************************************************************************/
int app_tm_write_rcv_only(void)
{
    UINT8 freq_enc = 0;

    APP_DEBUG0("");

    freq_enc = app_get_choice("Receive Frequency Encoded(2-80)");
    if (freq_enc < 2 || freq_enc > 80)
    {
        APP_ERROR0("app_tm_vsc_write_rcv_only: Invalid parameter");
        return -1;
    }
    else
    {
        APP_INFO1("app_tm_vsc_write_rcv_only: frequency (%d)", freq_enc + 2400);
    }
